...
use App\Enums\HashHmac;

use Symfony\Component\HttpFoundation\File\File;


enum Hash: string implements BaseEnumInterface, HasDefaultEnum {
    ...
    public function get_length(): ?int {
        ...
        return strlen($this->hash('test', false));
    }
    ...
    public static function supported(): array {
        return array_intersect(static::get_map_value(), hash_algos());
    }
    ...
    public function is_supported_hmac(): bool {
        $algo = $this->value;
        $hash_hmac = HashHmac::tryFrom($algo);
        
        if (! $hash_hmac) {
            return false;
        }
        
        return $hash_hmac->is_supported();
    }
    
    public function is_valid_file_for_hashing(string $filename): bool {
    ...
    public function equals(string $hash1, string $hash2): bool {
        ...
        return strlen($hash1) === strlen($hash2) && hash_equals($hash1, $hash2);
    }
    
    public function matches(string $plain, string $hashed): bool {
        return $this->is_supported()
            ? hash($this->value, $plain) === $hashed
            : false;
    }
    
    public function hash(string $data, bool $binary = false): ?string {
        return $this->is_supported()
            ? hash($this->value, $data, $binary)
            : null;
    }
    
    public function hash_file(string $filename, bool $binary = false): ?string {
        if (! $this->is_valid_file_for_hashing($filename) ) {
            return null;
        }
        ...
        if ($size > self::MAX_TOTAL_DEFAULT_STREAM) {
            return $this->hash_file_stream_auto($filename, $binary);
        }
        
        if ($size > self::MAX_SIZE_HASH_FILE) {
            return $this->hash_file_stream_auto($filename, $binary, true);
        }
        
        $result = hash_file($this->value, $filename, $binary);
        return $result === false 
            ? null 
            : $result;
    }
    
    private function hash_file_stream_auto(string $filename, bool $binary, bool $use_chunk = false): ?string {
