...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        Schema::table('signatures_type', function (Blueprint $table) {
            $table->foreign('id_signature')->references('id_signature')->on('signatures')->onDelete('cascade');
        });
        
        Schema::table('signatures_data', function (Blueprint $table) {
            $table->foreign('id_signature')->references('id_signature')->on('signatures')->onDelete('cascade');
        });
        
        Schema::table('signatures_file', function (Blueprint $table) {
            $table->foreign('id_signature_type')->references('id_signature_type')->on('signatures_type')->onDelete('cascade');
        });
        
        Schema::table('signatures_file', function (Blueprint $table) {
            $table->foreign('id_file_signature')->references('id_file_signature')->on('files_signature')->onDelete('cascade');
        });
